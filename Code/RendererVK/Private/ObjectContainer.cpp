...
void ObjectContainer::initializeMeshes(const std::vector<MeshData>& meshDataList)
{
    ...
    for (const MeshData& meshData : meshDataList)
    {
        ...
        meshInfo.indexCount   = (uint32)meshData.getNumIndices();
        ...
        meshInfo.firstIndex   = (uint32)(meshDataManager.uploadIndexData(pIndices, meshData.getNumIndices() * sizeof(RendererVKLayout::MeshIndex)) / sizeof(RendererVKLayout::MeshIndex));
...
{
    std::list<std::pair<const NodeData, int>> nodeDataParentIdxStack;
    std::vector<Transform> localSpaceNodes;

    nodeDataParentIdxStack.emplace_back(nodeData, 0);
    ...
    while (!nodeDataParentIdxStack.empty())
    {
        ...
        bool isRoot = nodeData == pAiNode;

        glm::vec3 pos, scale;
        glm::quat rot;
        pAiNode.getTransform(pos, scale, rot);
        ...
        const uint32 numChildren = pAiNode.getNumChildren();
        ...
        node.path = isRoot ? pAiNode.getName() : initialStateNodes[parentIdx].path + NODE_PATH_SEPARATOR + pAiNode.getName();

        if (pAiNode.getNumMeshes() > 0)
        {
            node.meshInfoIdx = (uint16)pAiNode.getMeshIndex(0);
            ...
            if (pAiNode.getNumMeshes() > 1)
                node.numChildren += (uint16)(pAiNode.getNumMeshes() - 1);

            for (uint32 i = 1; i < pAiNode.getNumMeshes(); ++i)
            {
                ...
                childNode.meshInfoIdx = (uint16)pAiNode.getMeshIndex(i);
        ...
        {
            nodeDataParentIdxStack.emplace_front(pAiNode.getChild(i), nodeIdx);
        }
    }
