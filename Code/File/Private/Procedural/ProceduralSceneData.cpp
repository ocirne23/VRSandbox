...
bool ProceduralSceneData::initialize(const char* shapeName, bool /*mergeNodes*/, bool /*preTransformVertices*/)
{
	...
	{
		assert(false && "ProceduralSceneData: unknown shape name. Use \"cube\", \"plane\" or \"sphere\".");
		return false;
	}

	// Mesh
	ProceduralMeshData& mesh = m_meshes.emplace_back();
	[[maybe_unused]] bool meshOk = mesh.initialize(shape, shapeName);
	...
	[[maybe_unused]] bool normalOk = normalTex.initialize(EProceduralTextureType::FlatNormal, 64, 64);
