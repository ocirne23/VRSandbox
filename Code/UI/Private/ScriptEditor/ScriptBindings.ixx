...
export struct BindingMember
{
	const char* name;
	DSLType type;
	const char* emit; // M6 template: e.g. "self->pos"
};
...
public:
	...
	// The member definition behind a receiver-type + name pair (nullptr = no such member) -- what stamps
	// MemberAccess::type at author/load time.
	const BindingMember* findMember(DSLType receiverType, const std::string& name) const;

	// The vec2/3/4 constructor builtin for a vector type (replaces the editor's remembered m_vecNFunc members).
	DSLSymbol* vectorBuiltin(DSLType vectorType) const;
...
private:
	...
	DSLSymbol* m_vec2Func = nullptr;
	DSLSymbol* m_vec3Func = nullptr;
