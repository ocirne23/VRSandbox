...
template<class T> static T vrArrGet(const ScriptContext* ctx, VrArray h, int i)
{ T v = T(); ctx->arrayGet(h, i, (int)sizeof(T), &v); return v; }
template<class T> static void vrArrSet(const ScriptContext* ctx, VrArray h, int i, const T& v)
{ ctx->arraySet(h, i, (int)sizeof(T), &v); }
template<class T> static void vrArrPush(const ScriptContext* ctx, Entity* self, VrArray* h, int kind, const T& v)
...
SCRIPT_EXPORT unsigned int ScriptDataLayoutId(void) { return 4236678032u; }
