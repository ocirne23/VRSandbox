	...
	boatControlUpdate.each([&](BoatControlComponent& boatComp, DynamicPhysicsComponent& physComp)
		{
			...
			if (boatComp.forwardBack != 0.0f)
				pBody->applyCentralImpulse(pBody->getInvInertiaTensorWorld().inverse() * pBody->getWorldTransform().getBasis().getColumn(2) * -boatComp.forwardBack * boatComp.moveSpeedMultiplier * (float)deltaSec);

			// Todo scale rotation speed based on forward velocity?
			if (boatComp.rightLeft != 0.0f)
				pBody->applyTorqueImpulse(pBody->getInvInertiaTensorWorld().inverse() * btVector3(0, -boatComp.rightLeft * boatComp.rotationSpeedMultiplier * (float)deltaSec, 0));
		});
}
