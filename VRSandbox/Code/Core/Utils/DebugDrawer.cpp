...
DebugDrawer* DebugDrawer::s_pDebugDrawer = nullptr;
...
{
	initialise();
	OGRE_ASSERT(s_pDebugDrawer == nullptr);
	s_pDebugDrawer = this;
}
...
{
	shutdown();
	s_pDebugDrawer = nullptr;
}

DebugDrawer& DebugDrawer::getSingleton(void)
{
	return *DebugDrawer::s_pDebugDrawer;
}
...
void DebugDrawer::build()
{
	...
	manualObject->begin("debug_draw", Ogre::OT_TRIANGLE_STRIP);
