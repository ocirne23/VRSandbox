...
* Components: `SceneComponent` (children + enabled), `RenderComponent` (RenderNode + local transform, static or skinned), `AnimatorComponent` (AnimationPlayer + AnimStateMachine from a .apl, ticks and pushes the skinning palette; gameplay via `stateMachine.setFloat/Bool/Trigger`, clip events via `onEvent`), `ScriptComponent` (runs a .scr each frame with the entity as `self`), `PhysicsComponent` (see Physics), `AudioComponent` (named triggerable sounds: `Component Audio` with `Sound <alias>` entries — `Path` + optional `Volume`/`Pitch`/`Loop`/`Relative`/`ReferenceDistance`/`MaxDistance`/`Rolloff`; `trigger(entity, alias, overrides)` plays one, playing spatial sounds follow the entity unless a position override pinned them, buffers World-cached per path; demo: physicsCube.pre), `ZoneComponent`/`CullingComponent` (mostly placeholders; `Zone.ixx` holds Morton-code entity-hash experiments)
