...
Dependency direction (`target_link_libraries`, all public): everything imports Core. `Animation`, `Physics`, `Script`, `Network`, `Audio` depend only on Core; `File` imports Animation; `RendererVK` imports File; `Entity` imports RendererVK + File + Script + Physics + Audio; `UI` imports Entity + RendererVK; `Input` imports UI; `App` links all.
...
## Entity
* The ECS. Inline component layout: an `Entity` (pos/scale/rot, parent, displayName, spawn template, refcount, `typeBits`) and its components live in ONE allocation — `typeBits` is a bitmask over the 8 component types (fixed order: Scene, Zone, Cull, Render, Animator, Script, Physics, Audio; `EComponentID` in `EntityDef.ixx`), and `getComponent<T>(entity)` / `hasComponent<T>` compute the byte offset from it at compile-time-known sizes (`Component.ixx`). Adding a component type means: new struct with `getId()`, entry in `EComponentID` + `inlineSizes` + `componentTypeName` + create/destroy/serialize switches in Entity.cpp, bump `MaxInlineComponentTypes`
...
* Components: `SceneComponent` (children + enabled), `RenderComponent` (RenderNode + local transform, static or skinned), `AnimatorComponent` (AnimationPlayer + AnimStateMachine from a .apl, ticks and pushes the skinning palette; gameplay via `stateMachine.setFloat/Bool/Trigger`, clip events via `onEvent`), `ScriptComponent` (runs a .scr each frame with the entity as `self`), `PhysicsComponent` (see Physics), `AudioComponent` (named triggerable sounds: `Component Audio` with `Sound <alias>` entries; each alias holds one or more `Path` clips — settings are per-`Path` children (`Volume`/`Pitch`/`Loop`/`Relative`/`ReferenceDistance`/`MaxDistance`/`Rolloff`) — plus an optional `Select` mode `Single`/`Random`/`RandomNoRepeat`/`Cycle` (`EAudioSelect`) that picks a clip per trigger; `trigger(entity, alias, overrides)` plays one, playing spatial sounds follow the entity unless a position override pinned them, buffers World-cached per path; demo: physicsCube.pre), `ZoneComponent`/`CullingComponent` (mostly placeholders; `Zone.ixx` holds Morton-code entity-hash experiments)
...
* A .scr file IS the generated C++ (`#include "ScriptAPI.h"`, entry points `OnSpawn`/`Update`/`OnDestroy`/`OnEvent`/`OnPhysicsEvent` + `ScriptEventCount`/`ScriptEventName`/`ScriptDataSize`) with the node graph appended as `//@graph` / `//@node` metadata comments — the NodeEditor regenerates the code from the graph, and the file is also hand-editable C++
...
* Contact/sensor events: shapes opt in via `ContactEvents`/`Sensor`; `PhysicsWorld::update` drains them per step into `getContactEvents()` (body userData pairs, plus a packed box3d `contactId` — 0 for sensor events); `dispatchPhysicsContactEvents()` (Entity, called from main loop) invokes `PhysicsComponent::onContact` and fires the script's `OnPhysicsEvent` entry point (the "On Physics Event" node — a fixed entry point like OnSpawn/Update, not a named-entry node like On Event) with the other entity, begin/sensor bools, and the contactId; `PhysicsWorld::getContactPoint(contactId, ...)` (script node "Get Contact Point") resolves it to a world-space hit position/normal, valid only until the next `physics.update()`
...
## Network
* Winsock UDP/TCP + a reliable-UDP game protocol (depends only on Core; Ws2_32 + Bcrypt linked PRIVATE). Module partitions `:Address`, `:Serialization`, `:Socket`, `:Crypto` (internal, nothing exported), `:Reliable`, re-exported from `Public/Network.ixx`. IPv4 only, wire format is raw little-endian bytes — no versioning/compat, both ends must match (`protocolId`)
* `:Serialization`: `NetWriter`/`NetReader` over a caller-provided byte span — trivially-copyable `write<T>`/`read<T>`, varint (LEB128 + zigzag), strings, unorm/snorm/`writeQuantized<UInt>(v, min, max)` float quantization; overflow-safe (check `overflowed()`), reads are zero-copy views
